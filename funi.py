    ...
    try:
        with open("./chat_data/chat_data.json", "r") as f:
            ...
            for i in range(len(f['data'])):
                if i <= 4:
                    chat_data.append({'role': f['data'][-i]['role'], 'content': f['data'][-i]['content']})

                match_numbers = len(set(f['data'][-i]['keyword']).intersection(set(keyword_list)))
                similarity_score = match_numbers * 2 / (len(set(f['data'][-i]['keyword'])) + len(set(keyword_list)))
                    
                print(f"\n\ncurrent data: {f['data'][-i]}\nsimilarity_score: {similarity_score}")
